...
import { addAuthorPubLink, addPublications, editPublication, getPublication, removeAuthorPubLink } from '../../actions/publicationActions'
...
const EditPublicationsScreen = ({ history, match }) => {
    ...
    const { loading: authorLoading, error: authorError, users: authorList } = useSelector(state => state.userList)
    const { loading: editLoading, error: editError, success: editSuccess } = useSelector(state => state.publicationEdit)
    const { loading: linkingLoading, error: linkingError, success: linkingSuccess } = useSelector(state => state.authorPubLink)
    const { loading: removeLinkingLoading, error: removeLinkingError, success: removeLinkingSuccess } = useSelector(state => state.authorPubRemoveLink)
    ...
    }, [dispatch, history, editSuccess, userInfo, match.params.id, publication, authorList, publication, linkingSuccess, removeLinkingSuccess])
